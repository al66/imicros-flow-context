...
module.exports = {
    ...
    actions: {
        ...
        updateToken: {
            ...
            params: {
                instanceId: { type: "uuid" },
                consume: { type: "array", optional: true, items: "object" },
                emit: { type: "array", optional: true, items: "object" }
            },
            async handler(ctx) {
                if (!ctx.params.consume && !ctx.params.emit) return false;
                ...
                if (ctx.params.consume) Promise.all(ctx.params.consume.map(async (e) => { 
                    query += "UPDATE " + this.tokenTable;
                    query += " SET tokens = tokens ";
                    query += "- {'" + await this.serializer.serialize(e) + "'}";
                    query += " WHERE owner = :owner AND instance = :instance; ";
                }));
                if (ctx.params.emit) Promise.all(ctx.params.emit.map(async (e) => { 
                    query += "UPDATE " + this.tokenTable;
                    query += " SET tokens = tokens ";
                    query += "+ {'" + await this.serializer.serialize(e) + "'}";
                    query += " WHERE owner = :owner AND instance = :instance; ";
                ...
                */
                try {
                    if (ctx.params.consume) {
                        query = "UPDATE " + this.tokenTable;
                        query += " SET tokens = tokens - {";
                        for (let i=0; i < ctx.params.consume.length; i++) {
                            if (i > 0) {
                                query += ",'" + await this.serializer.serialize(ctx.params.consume[i]) + "'";
                            } else {
                                query += "'" + await this.serializer.serialize(ctx.params.consume[i]) + "'";
                            }
                        }
                    ...
                    if (ctx.params.emit) {
                        ...
                        for (let i=0; i < ctx.params.emit.length; i++) {
                            if (i > 0) {
                                query += ",'" + await this.serializer.serialize(ctx.params.emit[i]) + "'";
                            } else {
                                query += "'" + await this.serializer.serialize(ctx.params.emit[i]) + "'";
                            }
                        }
        ...
        getToken: {
            ...
            async handler(ctx) {
                ...
                let query = "SELECT tokens FROM " + this.tokenTable;
                query += " WHERE owner = :owner AND instance = :instance;";
                let params = { 
                    owner: owner, 
                    instance: ctx.params.instanceId 
    ...
    methods: {
        ...
        async connect () {
            ...
            */
            ...
            query += " ( owner varchar, instance uuid, tokens set<text>, PRIMARY KEY (owner,instance) ) ";
