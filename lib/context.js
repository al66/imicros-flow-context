...
module.exports = {
    ...
    actions: {
        ...
        updateToken: {
            params: {
                instance: { type: "uuid" },
                consume: { type: "array", optional: true, items: "string" },
                emit: { type: "array", optional: true, items: "string" }
            },
            async handler(ctx) {
                let owner = await this.getOwnerId({ ctx: ctx, abort: true });
                if (!await this.isAuthorized({ ctx: ctx, ressource: {}, action: "token" })) throw new Error("not authorized");

                let query = "UPDATE " + this.contextTable;
                query += " SET tokens = tokens - {:consume} + {:emit} ";
